...
class OperatorSD(Operator):
    ...
    def local_run(self, op, *args, **kwargs):
        try:
            # log start
            print(f"{str(datetime.datetime.now())} operation start !!!!!!!!!!!!!!!!!!!!!!!!!!")
            clean_args = {k: v for k, v in kwargs.items() if k != 'input_image'}
...
if __name__ == '__main__':
    sd = OperatorSD()
    sd.local_run(sd, mode=['text2image'], user_id=['123'], params=[{'batch_size': 1, 'width': 512, 'height': 512}],
                 origin='localhost')

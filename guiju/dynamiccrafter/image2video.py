...
class Image2Video(object):
    def __init__(self, result_dir='./tmp/', gpu_num=1, resolution='256_256', save_fps=16) -> None:
        ...
        ckpt_path = '/media/zzg/GJ_disk01/pretrained_model/Doubiiu/DynamiCrafter_' + resolution.split('_')[
            1] + '/model.ckpt'
    ...
    def get_image(self, image, prompt, steps=50, cfg_scale=7.5, eta=1.0, fs=3, seed=123):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            ## inference
            batch_samples = batch_ddim_sampling(model, cond, noise_shape, n_samples=1, ddim_steps=steps, ddim_eta=eta,
                                                cfg_scale=cfg_scale)
            ## b,samples,c,t,h,w

# coding=utf-8
# @Time : 2023/5/31 下午3:18
# @File : global_var.py

html_label = {
    'lang_selection': {'ch': '中文', 'jp': '日本語'},
    'input_image_label': {'ch': '上传图片', 'jp': '画像をアップロード'},
    'batch_size_label': {'ch': 'Batch size', 'jp': '生成画像数'},
    'output_gender_label': {'ch': '输出性别', 'jp': '性別'},
    'output_gender_list': {'ch': ['女', '男'], 'jp': ['女', '男']},
    'output_viewpoint_label': {'ch': '输出视角', 'jp': 'viewpoint'},
    'output_viewpoint_list': {'ch': ['正面', '侧面', '背面'], 'jp': ['正', '侧', '背']},
    'output_age_label': {'ch': '输出年纪', 'jp': '年齢'},
    'output_age_list': {'ch': ['儿童', '青年', '中年'], 'jp': ['子ども', '青年', '中年']},
    'generate_btn_label': {'ch': '生成', 'jp': '生成'},
    'interrupt_btn_label': {'ch': '中断', 'jp': '中断'},
    'output_gallery_label': {'ch': '结果', 'jp': '結果'},
    'hint1': {'ch': '本システムは衣料品はの用可視化機能を持っています。画像内に表示されている衣料品を身に着けたマネキンやハンガーを実際の人物に変換します。これにより、ユーザーは実際にその衣料品がどのように見えるかをよりリアルに想像することができます。', 'jp': '本システムは衣料品はの用可視化機能を持っています。画像内に表示されている衣料品を身に着けたマネキンやハンガーを実際の人物に変換します。これにより、ユーザーは実際にその衣料品がどのように見えるかをよりリアルに想像することができます。'},
    'hint2': {'ch': '画像入力の注意点\n1,入力画像の縦横比は約2:1です。\n2,画像内のモデル、衣類、背景の色を異ならせてください。\n3,画像内の衣類をブロックしないでください。\n4,頭や手足を生成するために、画像の衣類の上下にスペースを空けてください。\n5,衣類が透けないようにしてください。', 'jp': '画像入力の注意点\n1,入力画像の縦横比は約2:1です。\n2,画像内のモデル、衣類、背景の色を異ならせてください。\n3,画像内の衣類をブロックしないでください。\n4,頭や手足を生成するために、画像の衣類の上下にスペースを空けてください。\n5,衣類が透けないようにしてください。'},
}

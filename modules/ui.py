...
def create_toprow_general(is_img2img):
    id_part = "txt2img_general"
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface_advanced:
        ...
        txt_prompt_img = gr.File(label="", elem_id="txt2img_advanced_prompt_image", file_count="single", type="binary", visible=False)

        with FormRow(variant='compact', elem_id="txt2img_advanced_extra_networks", visible=False) as extra_networks:
            from modules import ui_extra_networks
            extra_networks_ui = ui_extra_networks.create_ui(extra_networks, extra_networks_button, 'txt2img_advanced')

        with gr.Row().style(equal_height=False):
            with gr.Column(variant='compact', elem_id="txt2img_advanced_settings"):
                for category in ordered_ui_categories():
                    if category == "sampler":
                        steps, sampler_index = create_sampler_and_steps_selection(samplers, "txt2img_advanced")

                    elif category == "dimensions":
                        with FormRow():
                            with gr.Column(elem_id="txt2img_advanced_column_size", scale=4):
                                width = gr.Slider(minimum=64, maximum=2048, step=8, label="Width", value=512, elem_id="txt2img_advanced_width")
                                height = gr.Slider(minimum=64, maximum=2048, step=8, label="Height", value=512, elem_id="txt2img_advanced_height")

                            res_switch_btn = ToolButton(value=switch_values_symbol, elem_id="txt2img_advanced_res_switch_btn")
                            if opts.dimensions_and_batch_together:
                                with gr.Column(elem_id="txt2img_advanced_column_batch"):
                                    batch_count = gr.Slider(minimum=1, step=1, label='Batch count', value=1, elem_id="txt2img_advanced_batch_count")
                                    batch_size = gr.Slider(minimum=1, maximum=8, step=1, label='Batch size', value=1, elem_id="txt2img_advanced_batch_size")

                    elif category == "cfg":
                        cfg_scale = gr.Slider(minimum=1.0, maximum=30.0, step=0.5, label='CFG Scale', value=7.0, elem_id="txt2img_advanced_cfg_scale")

                    elif category == "seed":
                        seed, reuse_seed, subseed, reuse_subseed, subseed_strength, seed_resize_from_h, seed_resize_from_w, seed_checkbox = create_seed_inputs('txt2img_advanced')

                    elif category == "checkboxes":
                        with FormRow(elem_id="txt2img_advanced_checkboxes", variant="compact"):
                            restore_faces = gr.Checkbox(label='Restore faces', value=False, visible=len(shared.face_restorers) > 1, elem_id="txt2img_advanced_restore_faces")
                            tiling = gr.Checkbox(label='Tiling', value=False, elem_id="txt2img_advanced_tiling")
                            enable_hr = gr.Checkbox(label='Hires. fix', value=False, elem_id="txt2img_advanced_enable_hr")
                            hr_final_resolution = FormHTML(value="", elem_id="txtimg_hr_finalres", label="Upscaled resolution", interactive=False)

                    elif category == "hires_fix":
                        with FormGroup(visible=False, elem_id="txt2img_advanced_hires_fix") as hr_options:
                            with FormRow(elem_id="txt2img_advanced_hires_fix_row1", variant="compact"):
                                hr_upscaler = gr.Dropdown(label="Upscaler", elem_id="txt2img_advanced_hr_upscaler", choices=[*shared.latent_upscale_modes, *[x.name for x in shared.sd_upscalers]], value=shared.latent_upscale_default_mode)
                                hr_second_pass_steps = gr.Slider(minimum=0, maximum=150, step=1, label='Hires steps', value=0, elem_id="txt2img_advanced_hires_steps")
                                denoising_strength = gr.Slider(minimum=0.0, maximum=1.0, step=0.01, label='Denoising strength', value=0.7, elem_id="txt2img_advanced_denoising_strength")

                            with FormRow(elem_id="txt2img_advanced_hires_fix_row2", variant="compact"):
                                hr_scale = gr.Slider(minimum=1.0, maximum=4.0, step=0.05, label="Upscale by", value=2.0, elem_id="txt2img_advanced_hr_scale")
                                hr_resize_x = gr.Slider(minimum=0, maximum=2048, step=8, label="Resize width to", value=0, elem_id="txt2img_advanced_hr_resize_x")
                                hr_resize_y = gr.Slider(minimum=0, maximum=2048, step=8, label="Resize height to", value=0, elem_id="txt2img_advanced_hr_resize_y")

                    elif category == "batch":
                        if not opts.dimensions_and_batch_together:
                            with FormRow(elem_id="txt2img_advanced_column_batch"):
                                batch_count = gr.Slider(minimum=1, step=1, label='Batch count', value=1, elem_id="txt2img_advanced_batch_count")
                                batch_size = gr.Slider(minimum=1, maximum=8, step=1, label='Batch size', value=1, elem_id="txt2img_advanced_batch_size")

                    elif category == "override_settings":
                        with FormRow(elem_id="txt2img_advanced_override_settings_row") as row:
                            override_settings = create_override_settings_dropdown('txt2img_advanced', row)

                    elif category == "scripts":
                        with FormGroup(elem_id="txt2img_advanced_script_container"):
            ...
            txt2img_gallery, generation_info, html_info, html_log = create_output_panel("txt2img_advanced", opts.outdir_txt2img_samples)
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface_general:
        txt2img_prompt, txt2img_prompt_styles, txt2img_negative_prompt, submit, _, _, txt2img_prompt_style_apply, txt2img_save_style, txt2img_paste, extra_networks_button, token_counter, token_button, negative_token_counter, negative_token_button = create_toprow_general(is_img2img=False)
    ...
    interfaces = [
        (txt2img_interface_general, "general", "general"),
        (txt2img_interface_advanced, "advanced", "advanced"),
    ...
    visit(txt2img_interface_general, loadsave, "general")
    visit(txt2img_interface_advanced, loadsave, "advanced")

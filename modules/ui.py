...
def create_ui():
            ...
            with gr.Column(scale=4, min_width=600) as left_block:
                ...
                with gr.Row():
                    with gr.Column(scale=8):
                        txt2img_prompt = gr.Textbox(label="Prompt", elem_id=f"txt2img_prompt", show_label=False,
                                                    lines=1,
                                                    placeholder="Prompt (press Ctrl+Enter or Alt+Enter to generate)")
                        token_counter = gr.HTML(value="<span>0/75</span>", elem_id=f"{id_part}_token_counter",
                                                elem_classes=["token-counter"])
                        txt2img_negative_prompt = gr.Textbox(label="Negative prompt", elem_id=f"txt2img_neg_prompt",
                                                             show_label=False, lines=1,
                                                             placeholder="Negative prompt (press Ctrl+Enter or Alt+Enter to generate)")
                        negative_token_counter = gr.HTML(value="<span>0/75</span>",
                                                         elem_id=f"{id_part}_negative_token_counter",
                                                         elem_classes=["token-counter"])

                        with gr.Row(elem_id=f"{id_part}_tools", visible=False):
                            txt2img_paste = ToolButton(value=paste_symbol, elem_id="paste")
                            clear_prompt_button = ToolButton(value=clear_prompt_symbol,
                            ...
                            clear_prompt_button.click(
                                fn=lambda *x: x,
                                _js="confirm_clear_prompt",
                                inputs=[txt2img_prompt, txt2img_negative_prompt],
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with FormRow().style(equal_height=False):
            ...
            negative_token_button.click(fn=wrap_queued_call(update_token_counter),
                                        inputs=[img2img_negative_prompt, steps], outputs=[negative_token_counter])
    ...
    with gr.Blocks(analytics_enabled=False) as settings_interface:
        ...
        with gr.Tabs(elem_id="settings"):
            ...
            with gr.TabItem("Actions"):
                ...
                with gr.Row():
                    unload_sd_model = gr.Button(value='Unload SD checkpoint to free VRAM',
